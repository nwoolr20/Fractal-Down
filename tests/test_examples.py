...
from fractal_down.examples import (
    make_tiny_dag, make_weighted_dag, make_deep_dag, make_wide_dag, demo_run
)
...
def test_make_weighted_dag():
    ...
    for node_id in dag.postorder(root):
        ...
        if node.op is None or any(key in meta_dict for key in ['e', 'H', 'h', 'w', 'n', 'a']):
...
def test_weighted_dag_has_priorities():
    ...
    for node_id in reachable:
        ...
        for parent_id in node.inputs:
            ...
            assert parent_priority >= node_priority, \
    ...
    for node_id in reachable:
        ...
        if node.op is None:  # Leaf node
            assert priorities[node_id] >= 0.05, f"Leaf node {node_id} priority {priorities[node_id]} < 0.05"
...
def test_node_names_meaningful():
    ...
    for node_id in dag.postorder(root):
        ...
        if node.op is None:
            ...
            assert any(op in node.name.lower() 
                      for op in ['+', '*', 'add', 'mul', 'op'])
