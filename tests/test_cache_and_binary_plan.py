...
def test_cache_hit():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            plan1, path1, was_cached1 = get_or_build_plan(
                dag, c, 3, build_fn, None
            )
            ...
            plan2, path2, was_cached2 = get_or_build_plan(
                dag, c, 3, build_fn, None
            )
...
def test_corrupted_cache_file():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            plan2, path2, was_cached = get_or_build_plan(dag, a, 1, counting_build_fn, None)
