...
def test_memory_tracking_shows_differences():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            non_zero_deltas > 0
        ), f"Should have some non-zero memory deltas, got: {delta_rss_values[:10]}"

        print(
            f"✓ Memory tracking working: {non_zero_deltas}/{len(delta_rss_values)} runs show memory usage"
        )
...
def test_memory_stress_scenario():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if peak_rss_values:
            ...
            assert (
                avg_peak_mb > 10
            ), f"Memory stress should use >10MB, got {avg_peak_mb:.1f}MB"
            print(
                f"✓ Memory stress scenario creates substantial usage: {avg_peak_mb:.1f}MB average peak"
            )
