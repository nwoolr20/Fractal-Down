...
class Evaluator:
    ...
    def __init__(self, dag: DAG, inputs: Mapping[int, Any], 
    ...
    def run(self, plan: Plan, verify: bool = False) -> EvalResult:
        ...
        if verify:
            ...
            verified_value = self._compute_node(plan.root, verification_cache, plan.budget_nodes)
    ...
    def _compute_node(self, node_id: int, cache: OrderedDict[int, Any], 
                     budget: int) -> Any:
        """
        ...
        if node.op is None:
            ...
            if node_id not in self.inputs:
                raise ValueError(f"Missing input value for leaf node {node_id} ({node.name})")
    ...
    def _get_value(self, node_id: int, cache: OrderedDict[int, Any], budget: int) -> Any:
