...
def get_or_build_plan(dag: DAG, root: int, budget_nodes: int,
                     build_fn: Callable[[], Plan],
                     params: Optional[FractalParams] = None) -> Tuple[Plan, str, bool]:
    """
...
def _generate_fingerprint(dag: DAG, root: int, budget_nodes: int,
                         params: Optional[FractalParams]) -> str:
    """
...
def _canonicalize_dag(dag: DAG, root: int) -> str:
    ...
    for node_id in reachable:
        ...
        # Build node string
        node_str = (f"id={node.id}|name={node.name}|op={op_str}|"
