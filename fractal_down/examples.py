...
def make_weighted_dag() -> Tuple[DAG, int, Dict[int, Any]]:
    ...
    op1 = dag.add_op("x1+x2", operator.add, [x1, x2], 
                     {"e": 0.6, "H": 0.4, "w": 0.7, "n": 0.5, "a": 0.2})
    op2 = dag.add_op("x3*x4", operator.mul, [x3, x4],
                     {"e": 0.9, "H": 0.8, "w": 0.3, "n": 0.8, "a": 0.4})
    
    # Final operation
    root = dag.add_op("final", lambda a, b: a * b + a - b, [op1, op2],
...
def make_wide_dag() -> Tuple[DAG, int, Dict[int, Any]]:
    ...
    for i in range(0, len(input_ids), 2):
        if i + 1 < len(input_ids):
            op_id = dag.add_op(f"pair_{i//2}", operator.add, 
    ...
    while len(pair_ops) > 1:
        ...
        for i in range(0, len(pair_ops), 2):
            if i + 1 < len(pair_ops):
                op_id = dag.add_op(f"combine_{len(next_level)}", operator.mul,
