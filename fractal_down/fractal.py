...
def compute_node_priority(dag: DAG, root: int, params: Optional[FractalParams] = None) -> Dict[int, float]:
    """
    ...
    for node_id in reachable:
        ...
        H = float(meta_dict.get("H", meta_dict.get("h", 0.0)))  # Support both "H" and "h"
        w = float(meta_dict.get("w", 0.0))
        n = float(meta_dict.get("n", 0.0))
        a = float(meta_dict.get("a", 0.0))
        
        # Compute energy
        energy = params.alpha * e + params.beta * H + params.gamma * w + params.delta * n - params.kappa * a
...
def _enforce_parent_child_constraint(dag: DAG, nodes: list[int], 
                                   priorities: Dict[int, float]) -> Dict[int, float]:
    """
