...
class DAG:
    ...
    def add_leaf(self, name: str, meta: Optional[Dict[str, Any]] = None) -> int:
        ...
        node = Node(id=node_id, name=name, op=None, inputs=(), meta=sorted_meta)
    ...
    def add_op(
        ...
        node = Node(id=node_id, name=name, op=op, inputs=input_ids, meta=sorted_meta)
...
def _create_torch_dag_converter():
    ...
    try:
        ...
        def from_torch_module(
            module: torch.nn.Module, example_inputs: Tuple[Any, ...]
        ) -> Tuple[DAG, int, Dict[int, Any]]:
            """
            ...
            for fx_node in traced.graph.nodes:
                if fx_node.op == "placeholder":
                    ...
                    if fx_node.args and isinstance(fx_node.args[0], fx.Node):
                        ...
                        root_id = dag.add_op(
                            fx_node.name, lambda x: x, input_ids, {"fx_op": fx_node.op}
                        )
